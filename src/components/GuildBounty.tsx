...
class GuildBountyTarget extends React.Component<
  GuildBountyTargetProps,
  GuildBountyTargetState
> {
  ...
  render () {
    ...
    return (
      ...
      <ListItem disabled>
        <h4>
          {name}{' '}
          <IconButton onClick={() => this.setState({ isHidden: true })}>
            <DeleteIcon />
          </IconButton>
        </h4>
        {waypoint
          ? <div>
              ...
              <IconButton
                onClick={() => this.setState({ selectedWaypoint: undefined })}
              >
                <DeleteIcon />
              </IconButton>
            </div>
          : <WaypointSelection
              pois={(window as any).waypoints}
              mapKey={mapKey}
              onWaypointSelected={this.handleWaypointSelected}
            />}
      </ListItem>
    )
  }
